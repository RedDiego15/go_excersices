...
//		wg.Wait()
//	}
func getFromCache(value int, wg *sync.WaitGroup, cache *KeyStoreCache, t *testing.T) {
	defer wg.Done()
	cachedValue := cache.Get("Test" + strconv.Itoa(value))
	if t != nil {
		if cachedValue != "Test"+strconv.Itoa(value) {
			t.Errorf("Incorrect db response %v", value)
		}
	}
}
